...
async def create_subscription(
    ...
    db: Session = Depends(get_db),                     # ← CHANGED  (removed current_user)
):
    """
    Create a Stripe subscription (public).
    """
    user = db.query(User).filter(User.email == payload.email).first()  # ← CHANGED
    if not user:
        raise HTTPException(status_code=404, detail="User not found")   # ← CHANGED

    try:
        logger.info(f"POST /subscription for {user.email}")            # ← CHANGED
        ...
        sub = stripe.Subscription.create(
            ...
            metadata={"user_id": str(user.id)},                         # ← CHANGED
        )
        ...
        if sub.status == "active":
            ...
            user.subscribed = True                                     # ← CHANGED
            user.date_subscribed = now                                 # ← CHANGED
            user.date_subscription_expires = expires                   # ← CHANGED
            user.activated = True                                      # ← CHANGED
            db.commit()
            return SubscriptionResponse(
                subscribed=True,
                date_subscribed=now,
...
async def confirm_subscription(
    ...
    """
    Confirm a PaymentIntent after client-side SCA (public).
    """
    try:
        ...
        db.refresh(user)                                               # ← CHANGED
        return SubscriptionResponse(
            subscribed=user.subscribed,                                # ← CHANGED
            date_subscribed=user.date_subscribed,                      # ← CHANGED
            date_subscription_expires=user.date_subscription_expires,  # ← CHANGED
            requires_action=False,
        )
    except stripe.error.StripeError as e:
